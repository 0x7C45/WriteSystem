---
name: docx-preprocessor
description: Phase 0 原始资料预处理工具。处理用户提供的乱格式 DOCX，支持格式诊断、标点符号修复、格式统一，为后续阶段提供规范的输入文件。
phase: 0
...
**定位**: Phase 0 原始资料预处理工具

**使用场景**: 
- 用户提供的原始资料（参考范文、需求文档）格式混乱
- DOCX 文件存在中英文标点混用、段落格式不统一、字体不规范等问题
- 需要在进入主管线前对原始资料进行规范化处理

**与主管线的关系**:
- 本工具用于 Phase 0（订单提炼阶段）
- paper-order-analyst 可以调用此工具预处理用户提供的乱格式 DOCX
- 预处理后的文件更容易被后续 Agent 解析和分析

**注意**: 本工具不用于处理主管线生成的文件（Phase 2-3 的草稿和最终 DOCX 由 paper-formatter 负责排版）
